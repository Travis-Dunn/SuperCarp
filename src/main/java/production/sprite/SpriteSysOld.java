...
public class SpriteSysOld {
    ...
    public static boolean Init(int cap) {
        ...
        if (cap >= MIN_CAP && cap <= MAX_CAP) {
            SpriteSysOld.cap = cap;
        } else {
            SpriteSysOld.cap = DEF_CAP;
            LogSession(LogLevel.DEBUG, ErrStrCapOutOfBounds(cap, "cap",
                    DEF_CAP));
        }

        try {
            xyArr = new int[SpriteSysOld.cap];
            bitfieldArr = new int[SpriteSysOld.cap];
            freeList = new int[SpriteSysOld.cap];
    ...
    public static int Create(int x, int y, byte layer, short atlasIdx) {
        ...
        if (atlasIdx < 0 || atlasIdx > SpriteAtlasOld.MAX_ATLAS_IDX) {
            LogFatalAndExit(ErrStrValOOB(atlasIdx, "atlasIdx", 0,
                    SpriteAtlasOld.MAX_ATLAS_IDX));
            return INVALID_HANDLE;
        }
    ...
    public static final String CLASS = SpriteSysOld.class.getSimpleName();
