...
/**
 * Renders text to the framebuffer using a FontAtlasOld.
 * Supports word wrap and left/center/right alignment (left implemented).
 */
public final class TextRendererOld {
    ...
    public static int draw(byte[] fb, int fbWidth, int fbHeight,
        ...
        while (i < text.length()) {
            ...
            FontAtlasOld.Glyph glyph = font.getGlyph(c);
            ...
            if (lineWidth + glyphAdvance > maxWidth && lineStart < i) {
                ...
                for (int j = lineStart; j <= i; j++) {
                    if (j < text.length()) {
                        FontAtlasOld.Glyph g2 = font.getGlyph(text.charAt(j));
    ...
    private static void blitGlyph(byte[] fb, int fbWidth, int fbHeight,
                                  FontAtlasOld font, FontAtlasOld.Glyph glyph,
    ...
    public static final String CLASS = TextRendererOld.class.getSimpleName();
}