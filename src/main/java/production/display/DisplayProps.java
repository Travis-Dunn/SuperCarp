...
public final class DisplayProps {
    ...
    final int emulatedW, emulatedH;
    final int windowW, windowH;

    /**
     * Mode determines which fields are inputs and which are derived.
     * Fields marked (in) must be positive; (out) must be zero.
     * Fullscreen modes ignore borderless and windowW/H.
     *
     *                              pixelScale  emulatedW/H  windowW/H
    ...
    ErrStrInvalidCfgWindowedPixelEmulated(DisplayProps p) {
        return String.format("%s failed validation in mode " +
    ...
    private static String
    ErrStrInvalidCfgWindowedWindowEmulated(DisplayProps p) {
        return String.format("%s failed validation in mode " +
